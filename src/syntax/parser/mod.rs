...
impl<'a> Parser<'a> {
    ...
    fn ok_expr(&mut self,q:Expr)-> JsResult<Expr> {
        Ok(q)
    }
    ...
    fn parse_expr_unary(&mut self) -> JsResult<Expr> {
        ...
        loop {
            ...
            let mut thing2=try!(self.peek());
            expr = try!(match thing2 {
    ...
    fn parse_expr_array_literal(&mut self) -> JsResult<Expr> {
        ...
        if !try!(self.consume(Token::CloseBracket)) {
            loop {
                ...
                match self.peek().expect("parse_expr_array_literal error") {
